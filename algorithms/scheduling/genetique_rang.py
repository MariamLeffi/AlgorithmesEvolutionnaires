...
def selection_rang(population, durees):
    """Sélection par rang"""
    sorted_pop = sorted(population, key=lambda x: calculer_makespan(x, durees))
...
def algo_genetique_rang(durees, type_croisement="double", taille_pop=6, nb_generations=20):
    ...
    for _ in range(nb_generations):
        ...
        for _ in range(taille_pop):
            parent1 = selection_rang(population, durees)
            parent2 = selection_rang(population, durees)
