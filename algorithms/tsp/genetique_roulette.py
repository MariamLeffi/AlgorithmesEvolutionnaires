...
def distance_totale(sol, matrice_distances):
...
def selection_roulette(population, matrice_distances):
    """Sélection proportionnelle à la fitness"""
    distances = [distance_totale(ind, matrice_distances) for ind in population]
...
def croisement_double(parent1, parent2):
    """Croisement à deux points (Order Crossover)"""
    taille = len(parent1)
...
def algo_genetique_roulette(matrice_distances, type_croisement="double", taille_pop=6, nb_generations=20):
    """Algorithme génétique avec sélection par roulette"""
    nombre_villes = len(matrice_distances)
    population = generer_population(taille_pop, nombre_villes)
    meilleure_solution = min(population, key=lambda x: distance_totale(x, matrice_distances))
    meilleure_distance = distance_totale(meilleure_solution, matrice_distances)
    
    for generation in range(nb_generations):
        nouvelle_population = []
        
        for _ in range(taille_pop):
            parent1 = selection_roulette(population, matrice_distances)
            parent2 = selection_roulette(population, matrice_distances)
        ...
        current_best = min(population, key=lambda x: distance_totale(x, matrice_distances))
        current_best_distance = distance_totale(current_best, matrice_distances)
